...
// Code-behind stays dumb: parse input, call controller, push ViewState into grids.
public partial class InboundOrderView : UserControl
{
    private readonly InboundOrderController _controller;
    private readonly List<OrderLineInput> _pendingLines = new();

    public InboundOrderView(UserSession session)
    {
        InitializeComponent();
        _controller = new InboundOrderController(session);

        cboSupplier.ItemsSource = _controller.GetSuppliers();
    ...
    {
        if (cboSupplier.SelectedValue is not int supplierId)
        {
        ...
        try
        {
            int orderId = _controller.CreateOrder(
                txtOrderNo.Text.Trim(), supplierId,
                dtpStart.SelectedDate, dtpEnd.SelectedDate,
                string.IsNullOrWhiteSpace(txtNote.Text) ? null : txtNote.Text.Trim(),
                _pendingLines);

            MessageBox.Show($"Inbound order #{orderId} created. Receive tasks are waiting in the Work Queue.");
            _pendingLines.Clear();
